...
def append_op_costs_dfc(
    ...
    m.fs.electricity_revenue = Expression(
        expr=lmp * m.fs.power_dfc_to_grid,
        doc="Revenue generated by selling power to the grid [in $1000]",
    ) 
    ...
    m.fs.electricity_cost = Expression(    
        ...
        doc="Cost of electricity purchased from the grid [in $1000]",
    )

    m.fs.nitrogen_revenue = Expression(
        expr=m.fs.asu.nitrogen_revenue / 1000,
        doc="Revenue from nitrogen market [$1000]",
    )

    m.fs.argon_revenue = Expression(
        expr=m.fs.asu.argon_revenue / 1000,
        doc="Revenue from argon market [$1000]",
    )
